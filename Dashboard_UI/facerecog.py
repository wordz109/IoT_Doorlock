    ...
    if fetch_data():
        ...
        if found:
            ...
            if embedding is not None:
                ...
                label_status.config(text=f"Login berhasil! Selamat datang!", fg="green")
...
def update_camera():
    ...
    if ret:
        ...
        if user_embedding is not None:
            ...
            if recognized:
                status_text = f"Wajah cocok!"
                status_color = (0, 255, 0)
                status_verifikasi = "diizinkan"
            else:
                status_text = f"Wajah tidak cocok!"
                status_color = (255, 0, 0)
                status_verifikasi = "ditolak"

            # Upload status verifikasi ke Firebase
            try:
                username = entry_username.get()  # ambil username yang login
                ref_status = db.reference(f"status")
                ref_status.set({
